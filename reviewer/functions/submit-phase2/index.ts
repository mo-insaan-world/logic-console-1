...
const ALLOWED_CLASSIFICATIONS = new Set(['acceptable', 'harmful', 'lethal', 'na']);
const OUTPUTS_MAX  = 12;
const SPANS_MAX    = 4000;
const SPAN_TEXT_MAX = 40000;

interface OutputClassification { output_id: string; spans: Array<Record<string, unknown>>; worst_class: string; }
...
function validateOutputClassifications(raw: unknown): { cleaned: OutputClassification[]; worst: string } | { error: string } {
  ...
  for (let i = 0; i < raw.length; i++) {
    ...
    cleaned.push({ output_id: outputId, spans, worst_class: oL ? 'lethal' : oH ? 'harmful' : oA ? 'acceptable' : 'na' });
  }
  const worst = hasLethal ? 'LETHAL' : hasHarmful ? 'HARMFUL' : 'ACCEPTABLE';
  return { cleaned, worst };
}
...
Deno.serve(async (req) => {
  ...
  // Reviewer gate: no valid token => no write.
  const _denied = await requireReviewerToken(req, CORS_HEADERS);
  ...
  let existingRow: { id: string; case_id: string; ai_safety_rating: unknown; consultant_id?: string } | null = null;
  try {
    const checkRes = await fetch(
      `${SUPABASE_URL}/rest/v1/submissions?id=eq.${encodeURIComponent(body.submission_id)}&select=id,case_id,ai_safety_rating,consultant_id&limit=1`,
